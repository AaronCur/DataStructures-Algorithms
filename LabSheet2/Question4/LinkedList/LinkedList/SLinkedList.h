...
public:        
	...
	virtual void insertLast(T element) override;
...
{
	std::unique_ptr<SListNode<T>> newNode = std::make_unique<SListNode<T>>(element, this);
	// If list is empty, we will have one node only
	if (nullptr == m_head)
	{
	...
	else
	{
		...
		// next() returns unique_ptr, convert to raw
		// pointer with get()
...
void SLinkedList<T, InputIterator>::splice(InputIterator position, list<T> & x)
{
	...
	for (;iter != x.end(); iter++)
	{
		this->insertBefore(position, *iter);
		m_count;
	}
